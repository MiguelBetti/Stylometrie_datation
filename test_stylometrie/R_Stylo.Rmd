...
Nous allons tester le Burrows' Delta sur "Las doce figuras del mundo", écrit en collaboration par ce même auteur et Adolfo Bioy Casares, mais publié sur le pseudonyme de H. Bustos Domecq.
...
`Stylo` a besoin de savoir où chercher les données de notre "corpus" qu'il va utiliser, et celles-ci doivent **impérativement** obéir à trois règles simples:
...
Tous les labels (les noms des fichiers formant le corpus) avec la même chaîne de caractères avant l'underscore (dans notre cas, le nom chaque auteur), afin de simplifier la lecture des résultats.

Nous pouvons essayer plusieurs distances, fréquences, etc.
...
```
## 2.2 Contrôler la fiabilité des résultats: répéter l'opérations avec d'autres paramètres

Nous savons un premier cluster. Comment savoir s'il est fiable? Une première solution est de répéter le même clacul, en augmentant le nombre des mots les plus fréquents et en changeant la distance.

Selon Evert, Proisl, Jannidi, Reger, Pielström, Schöch, Vitt (2017), 5000 mots MFW avec Delta serait la méthode la plus efficace. Nous travaillons avec des textes courts (nouvelles, contes), essayons donc 2000 et 3000 mots.
...
Les résultat est plus clair… Nous pouvons également demander à `Stylo` de répéter l'analyse de cluster un certain nombre de fois entre 100 et 1000.
...
Il est possible de représenter graphiquement la somme de ces informations: il s'agit du *concensus tree* ("arbre de consensus", en français).

C'est l'occasion de changer encore une fois la méthode de calcul de distance, pour vérifier les performances de chacune. Nous utilisions jusqu'à présent le "Delta", essayons cette fois avec une distance de Manhattan
